...
namespace fl {
	...
	textures_manager::textures_manager() :
		textures_(size * size * 4),
		normalMaps_(size * size * 4),
		cursorX_(0),
		cursorY_(0),
		rowHeight_(0),
		texturesId_(0),
		...
		hasAutomaticUpdate_(true)
	{
		...
		const auto begin = reinterpret_cast<uint32_t*>(normalMaps_.data());
		const auto end = begin + size * size;
		uint32_t pixel = 0;
		pixel += 127;
		pixel += 127 << 8;
		pixel += 255 << 16;
	...
	texture_view textures_manager::store_from_data(uint8_t* texture, uint8_t* normalMap, int width, int height) {
		...
		const auto shift = (cursorX_ + cursorY_ * size) * 4;
		cursorX_ += width;
		...
		if (normalMap != nullptr) {
			...
			for (int i = 0; i < height; ++i) {
				memcpy(texureDst + i * size * 4, texture + i * width * 4, width * 4);
				memcpy(normalDst + i * size * 4, normalMap + i * width * 4, width * 4);
			}
		}
		else {
			const auto texureDst = textures_.data() + shift;
